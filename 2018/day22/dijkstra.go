...
type node struct {
	key  locationAndEquipment
	cost int
}

type locationAndEquipment struct {
	location
	equipment
}
...
func shortestPath(g caves, start, target locationAndEquipment) (path []locationAndEquipment, cost int, err error) {
	// ensure start and target are part of the graph
	if !g.contains(start.location) {
		err = fmt.Errorf("cannot find start %v in graph", start)
		return
	}
	if !g.contains(target.location) {
		err = fmt.Errorf("cannot find target %v in graph", target)
		return
	}

	explored := make(map[locationAndEquipment]bool)                 // set of nodes we already explored
	frontier := newQueue()                                          // queue of the nodes to explore
	previous := make(map[locationAndEquipment]locationAndEquipment) // previously visited node
	...
	for !frontier.isEmpty() {
		...
		for nKey, nCost := range g.getEdges(n.key) {
